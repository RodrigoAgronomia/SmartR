...
plot(rst$sim1[], col=clus$cluster) #faixas do cluster
...
## make clusters
clus = kmeans(rst[[1:2]][], 6)

## create raster with the clusters:
rst$clus = clus$cluster

clus$centers # visualiza Matrix

plot(rst$sim1[], rst$sim2[], col=clus$cluster) #faixas do cluster 2D
...
grd_regular=st_make_grid(field, n=8, what='centers')
...
grd_pol=st_make_grid(field2, cellsize = 10, what='polygons', square=FALSE )
points(st_coordinates(grd_pol),col='red')
